  ...
  <div id="popUp" :style="style" class="flex items-center justify-center">
    <div class="bg-white rounded-3xl p-6 overflow-auto">
      <div class="flex flex-col justify-around h-3/4">
        ...
        <p v-if="shipwreckData.imageSrc.length > 0 || shipwreckData.videoSrc.length > 0" class="text-sm">
          <span class="icon icon-camera text-xl mr-3"/>{{ $t("popup.videosAndPictures") }}
        </p>
        <div v-if="shipwreckData.imageSrc.length > 0 || shipwreckData.videoSrc.length > 0" class="flex flex-wrap">
          <video v-for="url in shipwreckData.videoSrc" :key="url" class="max-w-[50%] p-1" controls controlsList="nodownload">
            <source :src="url" type="video/mp4">
          </video>
          <img v-for="url in shipwreckData.imageSrc" :key="url" class="max-w-[50%] p-1 cursor-pointer" :src="url" @click="setCurrentImage(url)">
        </div>
      </div>
    </div>
  </div>
