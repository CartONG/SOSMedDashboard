<template>
    <div class="slider-container">
        <Slider
            ...
            connect: 'absolute z-1 top-0 right-0 transform-origin-0 transform-style-flat h-full w-full bg-red-500 cursor-pointer tap:duration-300 tap:transition-transform disabled:bg-gray-400 disabled:cursor-not-allowed',
            ...
            tooltip: 'absolute block text-sm font-semibold whitespace-nowrap py-1 px-1.5 min-w-5 text-center text-white rounded border border-red-500 bg-red-500 transform h:-translate-x-1/2 h:left-1/2 v:-translate-y-1/2 v:top-1/2 disabled:bg-gray-400 disabled:border-gray-400 merge-h:translate-x-1/2 merge-h:left-auto merge-v:-translate-x-4 merge-v:top-auto tt-focus:hidden tt-focused:block tt-drag:hidden tt-dragging:block',
...
const daysCount = Math.round((store.getState().maxDate.getTime() - store.getState().minDate.getTime()) / (1000 * 3600 * 24))
const startValues = [0, daysCount]

function formatTooltip (value: number) {
  const date = new Date(store.getState().minDate)
  date.setDate(date.getDate() + value)
  return date.toLocaleDateString()
}

function changeOnTooltip (values: [number, number]) {
  const minDate = new Date(store.getState().minDate)
  minDate.setDate(minDate.getDate() + values[0])
  const maxDate = new Date(store.getState().minDate)
  maxDate.setDate(maxDate.getDate() + values[1])
  store.filterData(minDate, maxDate)
}
</script>
